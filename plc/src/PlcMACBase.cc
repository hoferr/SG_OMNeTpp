...
void PlcMACBase::initializeQueueModule()
{
...
void PlcMACBase::initializeMACAddress()
{
...
void PlcMACBase::initializeNotificationBoard()
{
...
void PlcMACBase::initializeFlags()
{
...
void PlcMACBase::initializeStatistics()
{
    framesSentInBurst = 0;
    bytesSentInBurst = 0;
...
void PlcMACBase::registerInterface(double txrate)
{
    IInterfaceTable *ift = InterfaceTableAccess().getIfExists();
    if (!ift)
        return;

    interfaceEntry = new InterfaceEntry(NULL); // TODO Ramon: InterfaceEntry???
...
bool PlcMACBase::checkDestinationAddress(PlcFrame *frame)
{
...
void PlcMACBase::calculateParameters()
{
...
void PlcMACBase::printParameters()
{
    // Dump parameters
...
void PlcMACBase::processFrameFromUpperLayer(PlcFrame *frame)
{
...
void PlcMACBase::processMsgFromNetwork(cPacket *frame)
{
...
void PlcMACBase::frameReceptionComplete(PlcFrame *frame)
{
    int pauseUnits;
...
void PlcMACBase::processReceivedDataFrame(PlcFrame *frame)
{
    // bit errors
...
void PlcMACBase::processPauseCommand(int pauseUnits)
{
...
void PlcMACBase::handleEndIFGPeriod()
{
...
void PlcMACBase::handleEndTxPeriod()
{
...
void PlcMACBase::handleEndPausePeriod()
{
...
void PlcMACBase::processMessageWhenNotConnected(cMessage *msg)
{
...
void PlcMACBase::processMessageWhenDisabled(cMessage *msg)
{
	if (COMMENTS_ON) EV << "MAC is disabled -- dropping message " << msg << endl;
    delete msg;
}

void PlcMACBase::scheduleEndIFGPeriod()
{
	// CHANGE ------------------------------------------------------------------------------
	/*
...
// CHANGE ------------------------------------------------------------------------------
void PlcMACBase::scheduleEndIFGPeriod(int priority)
/*
...
void PlcMACBase::scheduleEndTxPeriod(cPacket *frame)
{
    scheduleAt(simTime()+frame->getBitLength()*bitTime, endTxMsg);
    transmitState = TRANSMITTING_STATE;
}

void PlcMACBase::scheduleEndPausePeriod(int pauseUnits)
{
...
bool PlcMACBase::checkAndScheduleEndPausePeriod()
{
...
void PlcMACBase::beginSendFrames()
{
    if (!txQueue.empty())
    {
...
void PlcMACBase::fireChangeNotification(int type, cPacket *msg)
{
    if (nb) {
...
void PlcMACBase::updateDisplayString()
{
    // icon coloring
    const char *color;
...
void PlcMACBase::updateConnectionColor(int txState)
{
    const char *color;
...
void PlcMACBase::receiveChangeNotification(int category, const cPolymorphic *)
{
