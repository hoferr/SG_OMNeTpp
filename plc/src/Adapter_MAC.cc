...
void Adapter_MAC::initialize()
{
    Adapter_MAC_Base::initialize();
    ...
    // check: datarate is forbidden with Adapter_MAC -- module's txrate must be used
    cGate *g = physOutGate;
    while (g)
    {
        cDatarateChannel *chan = dynamic_cast<cDatarateChannel*>(g->getChannel());
        if (chan && chan->par("datarate").doubleValue()>0)
            error("connection on gate %s has data rate set: using data rate with Adapter_MAC "
...
void Adapter_MAC::initializeTxrate()
{
    txrate = par("txrate");
}

void Adapter_MAC::startAutoconfig()
{
...
void Adapter_MAC::handleMessage (cMessage *msg)
{
    if (disabled)
    {
    ...
    // CHANGE ------------------------------------------------------------------------------
    if (dynamic_cast<EtherAutoconfig*>(msg)!=NULL && !autoconfigInProgress)
	{
...
void Adapter_MAC::processFrameFromUpperLayer(EtherFrame *frame)
{
    Adapter_MAC_Base::processFrameFromUpperLayer(frame);
...
void Adapter_MAC::processMsgFromNetwork(cPacket *msg)
{
    Adapter_MAC_Base::processMsgFromNetwork(msg);
...
void Adapter_MAC::handleEndIFGPeriod()
{
    Adapter_MAC_Base::handleEndIFGPeriod();
...
//    if (carrierExtension && frame->getByteLength() < GIGABIT_MIN_FRAME_WITH_EXT) // TODO: Changed by Ramon
    if (carrierExtension && frame->getByteLength() < GIGABIT_MIN_FRAME_BYTES_WITH_EXT) // TODO: Changed by Ramon
    {
        EV << "Performing carrier extension of small frame\n";
//        frame->setByteLength(GIGABIT_MIN_FRAME_WITH_EXT); // TODO: Changed by Ramon
...
void Adapter_MAC::startFrameTransmission()
{
...
void Adapter_MAC::handleEndTxPeriod()
{
    Adapter_MAC_Base::handleEndTxPeriod();
...
void Adapter_MAC::handleEndRxPeriod()
{
...
void Adapter_MAC::handleEndBackoffPeriod()
{
...
void Adapter_MAC::handleEndJammingPeriod()
{
...
void Adapter_MAC::sendJamSignal()
{
...
void Adapter_MAC::scheduleEndRxPeriod(cPacket *frame)
{
    scheduleAt(simTime()+frame->getBitLength()*bitTime, endRxMsg);
    receiveState = RECEIVING_STATE;
}

void Adapter_MAC::handleRetransmission()
{
...
void Adapter_MAC::finish()
{
    Adapter_MAC_Base::finish();
    /*
    simtime_t t = simTime();
...
void Adapter_MAC::updateHasSubcribers()
{
