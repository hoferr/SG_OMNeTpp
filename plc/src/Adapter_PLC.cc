...
Der Adapter_PLC ist der dem Netz zugewandte Teil eines BPLC-Steckermodems. Er ist im Grossen und Ganzen eine
Kopie des PlcMAC-Modules, welches sich aber nicht als Adapter "versteht", und Frames mit fremder MAC-Adresse
...
void Adapter_PLC::initialize()
{
    Adapter_PLC_Base::initialize();
	...
	Adapter_PLC_Base::JAM_SIGNAL_BYTES = par("JAM_SIGNAL_BYTES");
	Adapter_PLC_Base::MAX_PLC_FRAME = par("MAX_PLC_FRAME");
	Adapter_PLC_Base::MIN_PLC_FRAME = par("MIN_PLC_FRAME");
	Adapter_PLC_Base::PAUSE_BITTIME = par("PAUSE_BITTIME");
	Adapter_PLC_Base::PREAMBLE_BYTES = par("PREAMBLE_BYTES");
	Adapter_PLC_Base::SFD_BYTES = par("SFD_BYTES");

	Adapter_PLC_Base::CSMA_CA_MAX_ADDITIONAL_WAIT_TIME = par("CSMA_CA_MAX_ADDITIONAL_WAIT_TIME");
	Adapter_PLC_Base::CIFS = par("CIFS");
	Adapter_PLC_Base::RIFS = par("RIFS");
	Adapter_PLC_Base::PRIORITY_RESOLUTION_PERIOD = par("PRIORITY_RESOLUTION_PERIOD");
    ...
    // check: datarate is forbidden with Adapter_PLC -- module's txrate must be used
    cGate *g = physOutGate;
    while (g)
    {
        cDatarateChannel *chan = dynamic_cast<cDatarateChannel*>(g->getChannel());
        if (chan && chan->par("datarate").doubleValue()>0)
            error("connection on gate %s has data rate set: using data rate with Adapter_PLC "
...
void Adapter_PLC::initializeTxrate()
{
	// CHANGE -------------------------------------------------------------------
	/*
       Jedes Modem startet im ROBO-Mode (steht fuer "Robust OFDM"), d.h. alle Traeger
       werden nur mit BPSK betrieben, um erstmal auszuloten, was der Kanal so durchlaesst.
...
void Adapter_PLC::startAutoconfig()
{
...
void Adapter_PLC::handleAutoconfigMessage(cMessage *msg)
{
...
void Adapter_PLC::handleMessage (cMessage *msg)
{
	// CHANGE ------------------------------------------------------------------------------
...
void Adapter_PLC::processFrameFromUpperLayer(PlcFrame *frame)
{
    Adapter_PLC_Base::processFrameFromUpperLayer(frame);
...
void Adapter_PLC::processMsgFromNetwork(cPacket *msg)
{
	if (COMMENTS_ON) EV << "Sending msg from network to Adapter_PLCBase" << endl;
    Adapter_PLC_Base::processMsgFromNetwork(msg);
    if (COMMENTS_ON) EV << "Adapter_PLC_base finished processing." << endl;
...
void Adapter_PLC::handleEndIFGPeriod()
{
    Adapter_PLC_Base::handleEndIFGPeriod();
...
void Adapter_PLC::startFrameTransmission()
{
...
//		if(plc->getSrc().equals("00-00-00-00-00-00"))
		if(plc->getSrc() == MACAddress("00-00-00-00-00-00"))
		{
...
void Adapter_PLC::handleEndTxPeriod()
{
    Adapter_PLC_Base::handleEndTxPeriod();
...
void Adapter_PLC::handleEndRxPeriod()
{
...
void Adapter_PLC::handleEndBackoffPeriod()
{
...
void Adapter_PLC::handleEndJammingPeriod()
{
...
void Adapter_PLC::sendJamSignal()
{
	// Change ---------------------------------------------------------------------
	bubble("JAM DETECTED");
...
void Adapter_PLC::scheduleEndRxPeriod(cPacket *frame)
{
    scheduleAt(simTime()+frame->getBitLength()*bitTime, endRxMsg);
    receiveState = RECEIVING_STATE;
}

void Adapter_PLC::handleRetransmission()
{
    if (++backoffs > MAX_ATTEMPTS)
    {
    	if (COMMENTS_ON) EV << "Number of retransmit attempts of frame exceeds maximum, cancelling transmission of frame\n";
        delete txQueue.pop();
...
void Adapter_PLC::finish()
{
    Adapter_PLC_Base::finish();
    /*
    simtime_t t = simTime();
...
void Adapter_PLC::updateHasSubcribers()
{
    hasSubscribers = false;  // currently we don't fire any notifications
}

// CHANGE ----------------------------------------------------------------
void Adapter_PLC::readOutPreamble(cMessage *msg)
/*
...
/*
void Adapter_PLC::checkForVendorID(cMessage *msg)
{
...
void Adapter_PLC::schedulePolitenessPeriod()
{
...
void Adapter_PLC::applyPolitenessRules(cMessage *msg)
{
	PlcFrame *plc = check_and_cast<PlcFrame *>(msg);
...
void Adapter_PLC::removePolitenessRules()
{
...
int Adapter_PLC::priorityDistribution()
{
...
bool Adapter_PLC::compareMACAddresses (MACAddress *a)
{
...
void Adapter_PLC::sendSendingRequest()
{
	PlcSendRequestTDMA *sndreq = new PlcSendRequestTDMA ("SND_REQ");
	char buf[20];
	char *s = buf;
//	for (int j=0; j<MAC_ADDRESS_BYTES; j++, s+=3) sprintf(s,"%2.2X-", address.getAddressByte(j));
//	*(s-1)='\0';
	strcpy(s, address.str().c_str());
...
void Adapter_PLC::processFrameFromUpperLayerTDMA (PlcFrame *frame)
{
	...
	frame->setPriority(priorityDistribution());
...
void Adapter_PLC::manageReminder()
{
	...
	if(TDMAsendingEnabled == 1 && !txQueue.empty())
	{
		...
		// Get the frist message out of the queue
...
void Adapter_PLC::processMsgFromNetworkTDMA(cPacket *msg)
{
	readOutPreamble(msg);
...
void Adapter_PLC::handleMessageTDMA(cMessage *msg)
{
...
void Adapter_PLC::RXTXstateView()
{
...
void Adapter_PLC::cancelAllEvents()
{
	cancelEvent(endTxMsg);
	cancelEvent(endRxMsg);
...
void Adapter_PLC::requestCCoForQoSTraffic(int var_priority)
{
	if (CSMA_ONLY == 0)
	{
...
void Adapter_PLC::requestMoreBandwidth()
{
...
void Adapter_PLC::eraseChannelProperties()
{
	// if not CCo ...
	cModule *myParent = this->getParentModule();
	if(myParent->hasGate("PLC_side$o"))
	{
		cGate *myGate = myParent->gate("PLC_side$o");
...
void Adapter_PLC::sentStatisticalData()
{
	...
	if(temp>counter)
	{
		...
		simtime_t t = simTime();
		newData->setTotalCollisionTime(100*(totalCollisionTime/t));
		newData->setTotalSuccessfulRxTxTime(100*(totalSuccessfulRxTxTime/t));
		simtime_t totalChannelIdleTime = t - totalSuccessfulRxTxTime - totalCollisionTime;
		newData->setTotalChannelIdleTime(100*(totalChannelIdleTime/t));
